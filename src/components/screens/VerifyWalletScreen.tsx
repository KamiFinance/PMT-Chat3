...
export default function VerifyWalletScreen({ address, onVerified, onLogout }) {
  ...
  const verify = async (provider) => {
    ...
    try {
      ...
      try {
        ...
        if (permErr.code === 4001) { setErr('Connection rejected — please approve in your wallet.'); setVerifying(false); return; }
  ...
  const connectInjected = async () => {
    ...
    if (!window.ethereum) { setErr('No wallet found. Use WalletConnect below.'); setVerifying(false); return; }
    await verify(window.ethereum);
  };
    ...
    <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 16 }}>
      <div style={{ width: '100%', maxWidth: 400, background: 'var(--panel)', border: '1px solid var(--border)', borderRadius: 16, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.6 }}>
          Connect the wallet associated with this account to confirm ownership before continuing.
        </div>
        {/* Expected wallet */}
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <button onClick={connectInjected} disabled={verifying}
          style={{ padding: 13, background: 'var(--accent)', border: 'none', borderRadius: 10, color: '#0a0c14', fontWeight: 600, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: verifying ? 0.7 : 1 }}>
          {verifying
